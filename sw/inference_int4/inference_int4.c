...
#include <assert.h>
#include "weights_blk32_pkgUINT32_scaleE8M0_int4.h"
#include "image.h"
...
// INT4 Quantization
// =======================================
// Round to nearest at the header's layer 1 shift, clamp to -8..7, return the
// two's complement nibble. Pixels are non-negative, the sign path is here so
// the routine matches what the readout has to do.
int16_t int4_quantize(float value) {
    int sign = (value < 0.0f);
...
// =======================================
// Precomputed table to convert pixel bytes to INT4
static uint8_t pix_to_fp4[256];

static void build_pix_lut(void) {
    for (int v = 0; v < 256; v++) {
        pix_to_fp4[v] = (uint8_t)(int4_quantize((float)v / 255.0f) & 0xF);
    }
}
...
int main(void) {
    assert(rdout_shift[1] == 4 && rdout_shift[2] == 4);
